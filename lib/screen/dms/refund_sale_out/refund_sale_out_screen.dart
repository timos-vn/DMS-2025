...
import 'package:dms/screen/dms/refund_sale_out/refund_sale_out_bloc.dart';
...
class RefundSaleOutScreen extends StatefulWidget {
  ...
  const RefundSaleOutScreen({Key? key,required this.detailAgency, required this.codeTax,
    required this.percentTax, required this.tk,
    required this.nameCustomer,required this.codeCustomer,
    required this.phoneCustomer,required this.addressCustomer,
  }) : super(key: key);

  @override
  _RefundSaleOutScreenState createState() => _RefundSaleOutScreenState();
}

class _RefundSaleOutScreenState extends State<RefundSaleOutScreen> {

  late RefundSaleOutBloc _bloc;

  final nameAgencyController = TextEditingController();
  final addressAgencyController = TextEditingController();
  final phoneAgencyController = TextEditingController();
  ...
  void initState() {
    ...
    super.initState();
    nameAgencyController.text = widget.detailAgency.customerName.toString().trim();
    addressAgencyController.text = widget.detailAgency.address.toString().trim();
    phoneAgencyController.text = widget.detailAgency.phone.toString().trim();

    nameCustomerController.text = widget.nameCustomer.toString().trim();
    addressCustomerController.text = widget.addressCustomer.toString().trim();
    phoneCustomerController.text = widget.phoneCustomer.toString().trim();

    _bloc = RefundSaleOutBloc(context);
    _bloc.add(GetPrefsRefundSaleOutEvent(calculator: true));
    ...
    return Scaffold(
      body: BlocListener<RefundSaleOutBloc,RefundSaleOutState>(
        ...
        listener: (context,state){
          ...
          else if(state is AddNewRefundSaleOutSuccess){
            Utils.showCustomToast(context, Icons.check_circle_outline, 'Tạo phiếu thành công');
            DataLocal.listDetailSaleOutCompletedSave.clear();
            ...
            GestureDetector(
              onTap: (){
                if(DataLocal.listDetailSaleOutCompletedSave.isNotEmpty){
                  _bloc.add(AddNewRefundSaleOutEvent(
                    idCustomer: widget.codeCustomer.toString(),
                      phoneCustomer: widget.phoneCustomer.toString(),
                      addressCustomer: widget.addressCustomer.toString(),
                      codeTax: widget.codeTax,
                      tk: widget.tk.toString(),
                      idAgency: widget.detailAgency.customerCode.toString()
                  ));
                }else{
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Visibility(
            visible: DataLocal.listDetailSaleOutCompletedSave.isNotEmpty,
            child: SizedBox(
              height: (!_bloc.expanded || DataLocal.listDetailSaleOutCompletedSave.length == 1) ? 120 : 300,
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index){
                  return GestureDetector(
                    onTap: (){
                      setState(() {
                        if(DataLocal.listDetailSaleOutCompletedSave[index].isMark == false){
                          showDialog(
                              barrierDismissible: true,
                              context: context,
                              builder: (context) {
                                return InputQuantityPopupRefundOrder(
                                  title: 'Bạn chỉ được phép trả tối đa ${DataLocal.listDetailSaleOutCompletedSave[index].slCl!} SP',
                                  quantity: DataLocal.listDetailSaleOutCompletedSave[index].slCl??0,
                                );
                              }).then((value){
                            if(double.parse(value[0].toString()) > 0){
                              DataLocal.listDetailSaleOutCompletedSave[index].isMark = true;
                              DataLocal.listDetailSaleOutCompletedSave[index].slSt = value[0];
                          ...
                          DataLocal.listDetailSaleOutCompletedSave[index].isMark = false;
                          DataLocal.listDetailSaleOutCompletedSave[index].slSt = 0;
                          _bloc.add(CalculatorEvent());
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: Transform.scale(
                                ...
                                child: Checkbox(
                                  value:  DataLocal.listDetailSaleOutCompletedSave[index].isMark,
                                  onChanged: (b){
                                    setState(() {
                                      if(DataLocal.listDetailSaleOutCompletedSave[index].isMark == false){
                                        showDialog(
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Text("${DataLocal.listDetailSaleOutCompletedSave[index].sttRec.toString().trim()}/${DataLocal.listDetailSaleOutCompletedSave[index].maLo.toString().trim()}",
                                          style: TextStyle(color: Colors.black.withOpacity(0.7), fontSize: 12),
                                          textAlign: TextAlign.left,
                                        ),
                                      ],
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        Expanded(
                                            child: Text.rich(
                                              TextSpan(
                                                children: [
                                                  TextSpan(
                                                    text: '[${DataLocal.listDetailSaleOutCompletedSave[index].maVt.toString().trim()}] ',
                                                    style: const TextStyle(fontWeight: FontWeight.normal,fontSize: 12,color:  Color(
                                                        0xff555a55)),
                                                  ),
                                                  TextSpan(
                                                    text: DataLocal.listDetailSaleOutCompletedSave[index].tenVt.toString().trim(),
                                        ...
                                        (DataLocal.listDetailSaleOutCompletedSave[index].kmYn == 0) ?
                                        Column(
                                          children: [
                                            // Text(
                                            //   ((DataLocal.listDetailSaleOutCompletedSave[index].giaNt2??0))
                                            //       == 0 ? 'Giá đang cập nhật' : '${Utils.formatMoneyStringToDouble(DataLocal.listDetailSaleOutCompletedSave[index].giaNt2??0)} ₫'
                                            //   ,
                                            //   textAlign: TextAlign.left,
                                            //   style: TextStyle(color:
                                            //   ((DataLocal.listDetailSaleOutCompletedSave[index].ckNt??0)) == 0
                                            //       ?
                                            //   Colors.grey : Colors.red, fontSize: 10,
                                            //       decoration: ((DataLocal.listDetailSaleOutCompletedSave[index].ckNt??0)) == 0 ? TextDecoration.none : TextDecoration.lineThrough),
                                            // ),
                                            // const SizedBox(height: 3,),
                                            Visibility(
                                              visible: DataLocal.listDetailSaleOutCompletedSave[index].giaNt2! > 0,
                                              child: Text(
                                                '${Utils.formatMoneyStringToDouble((DataLocal.listDetailSaleOutCompletedSave[index].giaNt2??0))} ₫',
                                                textAlign: TextAlign.left,
                                        ...
                                        Padding(
                                          ...
                                          child: Icon(
                                            MdiIcons.gift,
                                            size:18,
                                            color: DataLocal.listDetailSaleOutCompletedSave[index].kmYn == 1 ? const Color(0xFF0EBB00) : Colors.transparent,
                                          ),
                                        ),
                                      ],
                                    ),
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        Flexible(
                                          child: Text(
                                            'Mã kho: ${(DataLocal.listDetailSaleOutCompletedSave[index].nameStore.toString().isNotEmpty && DataLocal.listDetailSaleOutCompletedSave[index].nameStore.toString() != 'null') ? DataLocal.listDetailSaleOutCompletedSave[index].nameStore : 'Đang cập nhật'}',
                                            textAlign: TextAlign.left,
                                        ...
                                        Row(
                                          ...
                                          children: [
                                            Text(
                                              DataLocal.listDetailSaleOutCompletedSave[index].kmYn == 0 ?'KH đặt:' : 'KL tặng:',
                                              style: TextStyle(color: DataLocal.listDetailSaleOutCompletedSave[index].kmYn == 0 ? Colors.black.withOpacity(0.7) : const Color(0xFF0EBB00), fontSize: 11),
                                              textAlign: TextAlign.left,
                                            ),
                                            const SizedBox(
                                              width: 5,
                                            ),
                                            Text("${DataLocal.listDetailSaleOutCompletedSave[index].soLuong??0} (${DataLocal.listDetailSaleOutCompletedSave[index].dvt.toString().trim()})",
                                              style: TextStyle(color:  DataLocal.listDetailSaleOutCompletedSave[index].kmYn == 0 ? blue : const Color(0xFF0EBB00), fontSize: 12),
                                              textAlign: TextAlign.left,
                                            ),
                                          ],
                                        ),
                                        ...
                                        Flexible(
                                          child: Row(
                                            ...
                                            children: [
                                              ...
                                              Text("${DataLocal.listDetailSaleOutCompletedSave[index].slSt} (${DataLocal.listDetailSaleOutCompletedSave[index].dvt.toString().trim()})",
                                          ...
                                          children: [
                                            Row(
                                              ...
                                              children: [
                                                ...
                                                Text("${DataLocal.listDetailSaleOutCompletedSave[index].slTra??0} (${DataLocal.listDetailSaleOutCompletedSave[index].dvt.toString().trim()})",
                                                  style:const TextStyle(color: blue, fontSize: 12),
                                                  textAlign: TextAlign.left,
                                                ),
                                              ],
                ...
                itemCount: DataLocal.listDetailSaleOutCompletedSave.length,
              )
            ),
          ),
          Visibility(
            visible: DataLocal.listDetailSaleOutCompletedSave.isNotEmpty && DataLocal.listDetailSaleOutCompletedSave.length > 1,
          ...
          Visibility(
            visible: DataLocal.listDetailSaleOutCompletedSave.isEmpty,
      ...
      children: [
        ...
        Padding(
          ...
          child: Column(
            ...
            mainAxisAlignment: MainAxisAlignment.start,
            children: [
              Text('Thông tin:',style: TextStyle(color: mainColor,fontSize: 13,fontStyle: FontStyle.italic),),
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Stack(
              children: [
                inputWidget(title:'Tên khách hàng',hideText: "Nguyễn Văn A",controller: nameCustomerController,textInputAction: TextInputAction.done, onTapSuffix: (){},note: true,isEnable: false),
                const Positioned(
                    top: 20,right: 10,
                    child: Icon(Icons.search_outlined,color: Colors.transparent,size: 20,) )
              ],
            ),
            inputWidget(title:"SĐT khách hàng",hideText: 'Đang cập nhật',controller: phoneCustomerController,textInputAction: TextInputAction.done, onTapSuffix: (){},note: true,isEnable: false),
            Stack(
              children: [
                inputWidget(title:'Địa chỉ khách hàng',hideText: "Vui lòng nhập địa chỉ KH",controller: addressCustomerController,textInputAction: TextInputAction.done, onTapSuffix: (){},note: true,isEnable: false),
                const Positioned(
                    top: 20,right: 10,
        ...
        child: Column(
          children: [
            Container(padding: const EdgeInsets.all(10),
              ...
              child: const Align(
                alignment: Alignment.centerLeft,
                child: Text('Thông tin Đại lý',style: TextStyle(color: Colors.black,fontSize: 13),),
              ),
            ),
            const SizedBox(height: 22,),
            Stack(
              children: [
                inputWidget(title:'Tên đại lý',hideText: "Đại lý A",controller: nameAgencyController,textInputAction: TextInputAction.done, onTapSuffix: (){},note: true,isEnable: false),
                const Positioned(
                    top: 20,right: 10,
                    child: Icon(Icons.search_outlined,color: Colors.transparent,size: 20,) )
              ],
            ),
            inputWidget(title:"SĐT đại lý",hideText: 'Đang cập nhật',controller: phoneAgencyController,textInputAction: TextInputAction.done, onTapSuffix: (){},note: true,isEnable: false),
            Stack(
              children: [
                inputWidget(title:'Địa chỉ đại lý',hideText: "Vui lòng nhập địa chỉ đại lý",controller: addressAgencyController,textInputAction: TextInputAction.done, onTapSuffix: (){},note: true,isEnable: false),
                const Positioned(
                    top: 20,right: 10,
                    child: Icon(Icons.edit,color: Colors.transparent,size: 20,))
              ],
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          customWidgetPayment('Thuế:',' 0 ₫',0,''),
          customWidgetPayment('Chiết khấu:','- 0 ₫',0,''),
          Padding(
