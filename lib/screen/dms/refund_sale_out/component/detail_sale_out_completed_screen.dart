...
class DetailSaleOutCompletedScreen extends StatefulWidget {
  final String sct;
  final String sttRec;
  ...
  const DetailSaleOutCompletedScreen({Key? key,required this.sct,required this.sttRec, required this.invoiceDate,
    required this.title, required this.codeTax, required this.percentTax,
    required this.detailAgency, required this.codeSell, required this.tk,
    required this.nameCustomer,required this.codeCustomer,
    required this.phoneCustomer,required this.addressCustomer,
  }) : super(key: key);

  @override
  _DetailSaleOutCompletedScreenState createState() => _DetailSaleOutCompletedScreenState();
}

class _DetailSaleOutCompletedScreenState extends State<DetailSaleOutCompletedScreen> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      ...
      if (maxScroll - currentScroll <= _scrollThreshold && !_hasReachedMax && _bloc.isScroll == true) {
        _bloc.add(GetDetailSaleOutCompletedEvent(isLoadMore:true,sctRec:widget.sttRec,invoiceDate: widget.invoiceDate,allowAddOrDeleteInList: false, addOrDeleteInList: false));
      }
    });
  }
      ...
      body: BlocListener<RefundSaleOutBloc,RefundSaleOutState>(
        ...
        listener: (context,state){
          if(state is GetPrefsRefundSaleOutSuccess){
            _bloc.add(GetDetailSaleOutCompletedEvent(sctRec:widget.sttRec,invoiceDate: widget.invoiceDate,allowAddOrDeleteInList: false, addOrDeleteInList: false));
          }
        },
          ...
          builder: (BuildContext context, RefundSaleOutState state){
            return Stack(
              children: [
                ...
                Visibility(
                  visible: state is GetListDetailSaleOutCompletedEmpty,
                  child: const Center(
  ...
  buildBody(BuildContext context,RefundSaleOutState state){
    ...
    if (state is GetListDetailSaleOutCompletedSuccess) {
                                ...
                                setState(() {
                                  if(_bloc.listDetailOrderCompleted[index].isMark == false){
                                    if(_bloc.listDetailOrderCompleted[index].soLuong! > _bloc.listDetailOrderCompleted[index].slTra!){
                                      showDialog(
                                        ...
                                        if(double.parse(value[0].toString()) > 0){
                                          ...
                                          if(widget.codeSell.isNotEmpty && widget.codeSell != 'null' && widget.codeSell != '' && DataLocal.lockRefundSaleOut == false){
                                            DataLocal.lockRefundSaleOut = true;
                                            DataLocal.sctSaleOut = widget.sct.toString();
                                            print('click ${DataLocal.sctSaleOut}');
                                            DataLocal.codeLockRefundSaleOut = widget.sttRec.toString().trim();
                                            DataLocal.codeTaxLockRefundSaleOut = widget.codeTax.toString().trim();
                                            DataLocal.percentTaxLockRefundSaleOut = widget.percentTax;
                                            DataLocal.codeSellLockRefundSaleOut = widget.codeSell.toString();
                                            DataLocal.tkRefundSaleOut = widget.tk.toString();
                                          }
                                          _bloc.listDetailOrderCompleted[index].slSt = value[0];
                                          DataLocal.listDetailSaleOutCompletedSave.add(_bloc.listDetailOrderCompleted[index]);
                                    ...
                                    if(DataLocal.listDetailSaleOutCompletedSave.isNotEmpty){
                                      DataLocal.listDetailSaleOutCompletedSave.removeWhere((element) => (element.sttRec.toString().trim() == _bloc.listDetailOrderCompleted[index].sttRec.toString().trim() && element.sttRec0.toString().trim() == _bloc.listDetailOrderCompleted[index].sttRec0.toString().trim()));
                                    }
                                    if(DataLocal.listDetailSaleOutCompletedSave.isEmpty && DataLocal.lockRefundSaleOut == true){
                                      DataLocal.lockRefundSaleOut = false;

                                      DataLocal.sctSaleOut = '';
                                      print('Unclick ${DataLocal.sctSaleOut }');
                                            ...
                                            children: [
                                                ...
                                                children: [
                                                  ...
                                                  Column(
                                                    children: [
                                                      ...
                                                      //   ((_bloc.listDetailOrderCompleted[index].giaNt2??0)) == 0
                                                      //       ?
                                                      //   Colors.grey : Colors.red, fontSize: 10,
                                                      //       decoration: ((_bloc.listDetailOrderCompleted[index].giaNt2??0)) == 0 ? TextDecoration.none : TextDecoration.lineThrough),
                                                      // ) ,
                                                      // const SizedBox(height: 3,),
                                                      Text(
                                                        '${Utils.formatMoneyStringToDouble((_bloc.listDetailOrderCompleted[index].giaNt2??0))} ₫',
                                                        textAlign: TextAlign.left,
                                              ...
                                              Row(
                                                ...
                                                children: [
                                                  Flexible(
                                                    child: Text(
                                                      'SL còn lại: ${(_bloc.listDetailOrderCompleted[index].slCl.toString().isNotEmpty && _bloc.listDetailOrderCompleted[index].slCl.toString() != 'null') ? _bloc.listDetailOrderCompleted[index].slCl : '0'}',
                                                      textAlign: TextAlign.left,
