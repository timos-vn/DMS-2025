...
class SearchProvinceBloc extends Bloc<SearchProvinceEvent,SearchProvinceState>{
  ...
  List<ListAreaResponseData> _searchResultsArea = <ListAreaResponseData>[];
  List<ListAreaResponseData> get searchResultsArea => _searchResultsArea;

  List<ListStoreFormResponseData> _searchResultsStoreForm = <ListStoreFormResponseData>[];
  List<ListStoreFormResponseData> get searchResultsStoreForm => _searchResultsStoreForm;

  List<ListTypeStoreResponseData> _searchResultsTypeStore = <ListTypeStoreResponseData>[];
  List<ListTypeStoreResponseData> get searchResultsTypeStore => _searchResultsTypeStore;
  ...
  Future<SearchProvinceState> handleCallApi(int pageIndex,String idProvince, String idDistrict,int typeGetList, String keysText, String idArea) async {
   ...
   else if(typeGetList == 1) {
     SearchProvinceState state = _handleLoadListArea(await _networkFactory!.getListArea(_accessToken!,pageIndex,_maxPage,keysText), pageIndex);
     return state;
   }
   else if(typeGetList == 2){
     SearchProvinceState state = _handleLoadListStoreForm(await _networkFactory!.getListStoreForm(_accessToken!,pageIndex,_maxPage,keysText), pageIndex);
     return state;
   }else {
     SearchProvinceState state = _handleLoadListTypeStore(await _networkFactory!.getListTypeStore(_accessToken!,pageIndex,_maxPage,keysText), pageIndex);
     return state;
   }
  }
  ...
  SearchProvinceState _handleLoadListArea(Object data, int pageIndex) {
    if (data is String) return SearchProvinceFailure('Úi, ${data.toString()}');
    try {
      ListAreaResponse response = ListAreaResponse.fromJson(data as Map<String,dynamic>);
      _maxPage = 20;
      List<ListAreaResponseData> list = response.data!;
      if (!Utils.isEmpty(list) && _searchResultsCommune.length >= (pageIndex - 1) * _maxPage + list.length) {
        _searchResultsArea.replaceRange((pageIndex - 1) * maxPage, pageIndex * maxPage, list);
      } else {
        if (_currentPage == 1) {
          _searchResultsArea = list;
        } else {
          _searchResultsArea.addAll(list);
        }
      }
      if (Utils.isEmpty(_searchResultsArea)) {
        return GetListProvinceEmpty();
      } else {
        isScroll = true;
      }
  ...
  SearchProvinceState _handleLoadListStoreForm(Object data, int pageIndex) {
    if (data is String) return SearchProvinceFailure('Úi, ${data.toString()}');
    try {
      ListStoreFormResponse response = ListStoreFormResponse.fromJson(data as Map<String,dynamic>);
      _maxPage = 20;
      List<ListStoreFormResponseData> list = response.data!;
      if (!Utils.isEmpty(list) && _searchResultsStoreForm.length >= (pageIndex - 1) * _maxPage + list.length) {
        _searchResultsStoreForm.replaceRange((pageIndex - 1) * maxPage, pageIndex * maxPage, list);
      } else {
        if (_currentPage == 1) {
          _searchResultsStoreForm = list;
        } else {
          _searchResultsStoreForm.addAll(list);
        }
      }
      if (Utils.isEmpty(_searchResultsStoreForm)) {
        return GetListProvinceEmpty();
      } else {
        isScroll = true;
      }
  ...
  SearchProvinceState _handleLoadListTypeStore(Object data, int pageIndex) {
    if (data is String) return SearchProvinceFailure('Úi, ${data.toString()}');
    try {
      ListTypeStoreResponse response = ListTypeStoreResponse.fromJson(data as Map<String,dynamic>);
      _maxPage = 20;
      List<ListTypeStoreResponseData> list = response.data!;
      if (!Utils.isEmpty(list) && _searchResultsTypeStore.length >= (pageIndex - 1) * _maxPage + list.length) {
        _searchResultsTypeStore.replaceRange((pageIndex - 1) * maxPage, pageIndex * maxPage, list);
      } else {
        if (_currentPage == 1) {
          _searchResultsTypeStore = list;
        } else {
          _searchResultsTypeStore.addAll(list);
        }
      }
      if (Utils.isEmpty(_searchResultsTypeStore)) {
        return GetListProvinceEmpty();
      } else {
        isScroll = true;
      }
