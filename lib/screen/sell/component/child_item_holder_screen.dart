...
class ChildItemHolderScreen extends StatefulWidget {
  ...
  const ChildItemHolderScreen({Key? key, required this.listOrder,required this.i,required this.userId,required this.bloc,this.indexSelect}) : super(key: key);

  @override
  State<ChildItemHolderScreen> createState() => _ChildItemHolderScreenState();
}

class _ChildItemHolderScreenState extends State<ChildItemHolderScreen> {
  ...
  void initState() {
    ...
    widget.bloc.add(GetListHistoryOrder(status: widget.i,dateFrom: Const.dateFrom, dateTo: Const.dateTo,userId:  widget.userId, typeLetterId: 'ITEMHOLDER'));
                        ...
                        return GestureDetector(
                          onTap: () {
                            PersistentNavBarNavigator.pushNewScreen(context, screen: GetItemHolderDetail(
                              sttRec: widget.listOrder[index].sttRec.toString(),
                              ...
                              child: Column(
                                ...
                                children: [
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            Row(
                                              children: [
                                                Expanded(child: Text('${widget.listOrder[index].tenKh}', style: const TextStyle(color: Colors.black,fontWeight: FontWeight.bold,fontSize: 13),)),
                                                Text.rich(
                                                  TextSpan(
                                                    children: [
                                                      const TextSpan(
                                                      ...
                                                      TextSpan(
                                                        text: Utils.parseStringDateToString('${widget.listOrder[index].ngayCt}', Const.DATE_SV, Const.DATE_FORMAT_1),
                                                ...
                                                Expanded(
                                                  child: Row(
                                                    children: [
                                                      ...
                                                      Text(widget.listOrder[index].dienThoaiKH??'null', style: const TextStyle(color: Colors.grey,fontSize: 12),),
                                                    ],
                                                  ),
                                                ),
                                                Text.rich(
                                                  TextSpan(
                                                    children: [
                                                      ...
                                                      TextSpan(
                                                        text: Utils.parseStringDateToString('${widget.listOrder[index].dateEstDelivery}', Const.DATE_SV, Const.DATE_FORMAT_1),
                                                        style: const TextStyle(fontWeight: FontWeight.normal,fontSize: 12),
                                                      ),
                                                    ],
                                                  ),
                                            ...
                                            Row(
                                              children: [
                                                ...
                                                const SizedBox(width: 3,),
                                                Expanded(child: Text('${widget.listOrder[index].diaChiKH}', style:const  TextStyle(color: Colors.grey,fontSize: 12),maxLines: 1,overflow: TextOverflow.ellipsis,)),
                                              ],
                                            ),
                                          ],
                                        ),
                                  ...
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text('Tổng tiền: ${Utils.formatMoneyStringToDouble(widget.listOrder[index].tTtNt??0)} VNĐ', style: const TextStyle(color: Colors.red,fontSize: 12),),
                                      Text('${widget.listOrder[index].statusname}', style: const TextStyle(color: Colors.black,fontSize: 12),),
                                    ],
                                  ),
                                ],
                              ),
